...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"`🇲​🇴​ℕ𝕊𝕋𝔼ℝ 𝗜𝗦 𝗦𝗧𝗔𝗥𝗧𝗜𝗡𝗚.`")
        try:
            await X1.disconnect()
        except Exception:
            pass
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"»🇲​🇴​ℕ𝕊𝕋𝔼ℝ 𝗡𝗘𝗪 𝗦𝗨𝗗𝗢 𝗨𝗦𝗘𝗥 𝗔𝗗𝗗𝗘𝗗__")
        target = ""
        ...
        if str(target) in sudousers:
            await ok.edit(f"𝕆𝕩𝕪𝕘𝕖𝕟 ꭙ 𝕊𝕡𝕒𝕞🫧 𝗦𝗨𝗗𝗢 𝗨𝗦𝗘𝗥. !!")
        else:
            if len(sudousers) > 0:
        ...
        await event.reply("»🇲​🇴​ℕ𝕊𝕋𝔼ℝ 𝗢𝗪𝗡𝗘𝗥 𝗖𝗔𝗡 𝗚𝗜𝗩𝗘 𝗦𝗨𝗗𝗢...")
...
async def show_sudo_users(event):
    if event.sender_id == OWNER_ID:
        sudo_users_list = " 🇲​🇴​ℕ𝕊𝕋𝔼ℝ 𝗖𝗨𝗥𝗥𝗘𝗡𝗧 𝗦𝗨𝗗𝗢 𝗨𝗦𝗘𝗥𝗦 𝗟𝗜𝗦𝗧:\n"
        for user_id in SUDO_USERS:
            sudo_users_list += f"- {user_id}\n"
        await event.reply(sudo_users_list)
    else:
        await event.reply("🇴𝗡𝗟𝗬 𝗙𝗢𝗥 🇲​🇴​ℕ𝕊𝕋𝔼ℝ 𝗢𝗪𝗡𝗘𝗥.")
